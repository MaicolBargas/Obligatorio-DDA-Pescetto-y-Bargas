...
public class MetodosArbitro {
    ...
    public static void MostrarArbitroPorPartido(int pPartido){
        try {
            ...
            while (rs.next()){
                ...
                unArbitro.setApellido(rs.getString(3));
                unArbitro.setPuesto(rs.getString(4));
                System.out.println(unArbitro.toString1());
            }
        }
        catch (SQLException ex){
            System.out.println(ex.toString());
    ...
    public static int SeleccionarArbitroPorPuesto(String pPuesto){
        Scanner scanner = new Scanner(System.in);
        MostrarArbitroPorPuesto(pPuesto);
        System.out.println("Escriba la ci del arbitro");
        int arbitro = scanner.nextInt();
        return arbitro;
    }

    public static Arbitro BuscarArbitro(int pCi){
        try {
            Connection con = Conexion.getConexion();
            PreparedStatement ps = con.prepareStatement("SELECT * FROM arbitro where ci = ?");
            ps.setInt(1,pCi);
            ResultSet rs = ps.executeQuery();
